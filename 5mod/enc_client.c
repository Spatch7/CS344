...
// Acts as a client to communicate with enc_server
// sends a textfile, key, port, and outputFile for the server to encrpyt and return.
// example input: ./enc_client plaintext1 mykey 55123 > ciphertext1
...
int main(int argc, char *argv[]) {
  ...
  // Compare input and key for validation
  ...
  if (secure_connection(socketFD) == 1)
  {
    ...
    {
      // Write to server
      charsRead = send(socketFD, input_text_ct + current_char, strlen(input_text_ct), 0); 
      if (charsRead < 0) 
      {
    ...
    memset(read_buffer, '\0', sizeof(read_buffer)); // Clear out the read_buffer again for reuse
    charsRead = recv(socketFD, read_buffer, sizeof(read_buffer) - 1, 0); // Read data from the socket, leaving \0 at end
    if (charsRead < 0) 
    {
