...
// example input: ./enc_server 55124
...
void decrypt_item(int connectionSocket);
  ...
  while(parent_pid == getpid())
  {
    ...
    if (count_sockets_used < 5)
    {
      ...
      switch (pid)
      {
        ...
        case 0:
          // If handshake with client successful, decrpyt text
          if (secure_connection(connectionSocket) == 1)
          {
            decrypt_item(connectionSocket);
...
// Creates a handshake with the dec_client, returns 1 success
int secure_connection (int connectionSocket)
{
  // Only allow 1000 characters, 1024 according to grading script
  char* charsRead;
  int chars_total;
  int char_count = 0;
...
void decrypt_item(int connectionSocket)
{
  ...
  output_text = (char* )calloc(input_text_size, sizeof(char));
  memset(output_text, '\0', input_text_size);
  ...
  // Request output_text from client
	...
	// Receive output_text from client
	charsRead = recv(connectionSocket, output_text, input_text_size - 1, 0);
	if (charsRead < 0)
	{
		fprintf(stderr, "SERVER: Error receiving output_text");
	}
	...
	for (int i = 0; i < strlen(output_text); i++) 
  {
    ...
    // If standard character
		else if (isupper(output_text[i]) != 0) {
			output_text[i] -= 64;
			output_text[i] -= key[i] - 64;
  ...
  // Return encrpytion
  while (char_count < strlen(output_text)) 
  {
    charsRead = send(connectionSocket, output_text + char_count, strlen(output_text), 0); 
    if (charsRead < 0) 
    {
			error("SERVER: Failed to return encrpyted plaintext");
      // fprintf(stderr, "Error: server issue returning encrpyted output_text");
      break;
    }
    if (charsRead <= strlen(output_text) && charsRead >= 0) 
		{
      char_count += charsRead;
    }
  }
